...
| max.poll.interval.ms  | 300000 | 기본값 : 300000, 컨슈머는 주기적으로 poll() 을 호출하여 토픽으로부터 레코드를 가져오며, poll() 호출 후 최대 5분간 poll() 호출이 없다면 컨슈머에 문제가 있는 것으로 판단하여 리밸런싱 동작 발생 (HeartBeat 쓰레드가 poll 호출 간격 측정) |
